...
namespace esyasoft.mobility.CHRGUP.service.ocpp.State
{
    public class CanonicalSessionStore
    {
        ...
        //    }
        //}
        // Key = $"{chargerId}:{evseId}"
        private static readonly ConcurrentDictionary<string, CanonicalChargeSession> _sessions = new();

        private static string Key(string chargerId, int evseId) => $"{chargerId}:{evseId}";

        public static CanonicalChargeSession GetOrCreate(string chargerId, OcppProtocol protocol, int evseId)
        {
            return _sessions.GetOrAdd(Key(chargerId, evseId), _ => new CanonicalChargeSession
            {
                ChargerId = chargerId,
                EvseId = evseId,
                Protocol = protocol,
        ...
        public static void Remove(string chargerId, int evseId)
        {
            _sessions.TryRemove(Key(chargerId, evseId), out _);
        }
    }
}
